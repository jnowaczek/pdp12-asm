...
def symbol_lookup(name):
    ...
    if name == '.':
        return mem_origin + instruction_count
    if mode == 'lmode':
...
def p_set_origin(p):
    ...
    global mem_origin, instruction_count
...
def p_assignment(p):
    ...
    if p[1] not in user_symbols:
        if p[2] == ',':
            user_symbols.update({p[1]: mem_origin + instruction_count})
        elif p[2] == '=':
...
def p_term(p):
    """term : NUMBER
            | DOT"""
    if p[1] == '.':
        p[0] = mem_origin + instruction_count
    else:
        p[0] = int(p[1], radix)
